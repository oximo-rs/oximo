...
/// Build the flowsheet model, solve it with `solver`, and print the solution.
/// Generic over the [`Solver`] backend and its options type.
#[cfg(any(feature = "gams", feature = "gurobi"))]
fn solve_and_report<S: Solver>(
    ...
    m.maximize(11.0 * c1 - 3.5 * y1 - y2 - 1.5 * y3 - b2 - 1.2 * b3 - 1.8 * (a2 + a3) - 7.0 * bp);
...
#[cfg(feature = "gams")]
fn run_gams() -> Result<(), Box<dyn std::error::Error>> {
...
#[cfg(feature = "gurobi")]
fn run_gurobi() -> Result<(), Box<dyn std::error::Error>> {
